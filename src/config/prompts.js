...
[
  {
    ...
    // ── Quest Testing Questions ─────────────────────────────────────────────────
    // 3 distinct MCQs for a specific quest module. Stored separately from quest
    // content and scored independently from trial question_bank questions.
    quest_questions: ({ level, period, subject, moduleTitle, topics, curriculumChunks }) => `You are a T&T primary school exam writer. Generate exactly 3 multiple-choice assessment questions for the quest module below. Return ONLY a valid JSON array, no other text.
...
${curriculumChunks ? `\nCURRICULUM NOTES:\n${curriculumChunks.slice(0, 800)}\n` : ''}
RULES:
1. Exactly 3 questions — one easy, one medium, one hard in that order
...
5. Each question must test genuine understanding, not just recall
6. explanation = why the correct answer is right (shown after submission)
7. tip = a brief hint that guides thinking without revealing the answer
8. cognitive_level: 'knowledge' | 'comprehension' | 'application' | 'analysis'
